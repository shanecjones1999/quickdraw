...
export function BowmanPlayer({ roomCode, playerName }: Props) {
    ...
    const [flyingShot, setFlyingShot] = useState<{
        angle: number;
        power: number;
    } | null>(null);
    ...
    function computeDrag(clientX: number, clientY: number) {
        ...
        const power = Math.min(dist / (rect.width * 0.3), 1) * 100;
    ...
    const onGameOver = useCallback(
        ({ results: r }: { results: BowmanResult[] }) => {
            setResults(r);
            setPhase("results");
        },
        [],
    ...
    const onBowmanSync = useCallback(
        ({
            shots: nextShots,
            ...
            setLastResult(nextShots.at(-1) ?? null);
            setShowFlash(false);
            setPhase(done ? "done" : "playing");
        },
        [],
    );

    useSocket("bowman:sync", onBowmanSync as never);
                        ...
                        <div className={styles.shotDots}>
                            {Array.from({ length: MAX_SHOTS }, (_, i) => {
                                ...
                                return s ? (
                                    ...
                                    <div
                                        key={i}
                                        className={`${styles.shotDot} ${styles.shotDotEmpty}`}
                                    >
                                        —
                                    </div>
                                );
